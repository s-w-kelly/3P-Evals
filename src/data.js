  ...
  analysisInsights: [
     {
      ...
      content: `
        <p>In <a href="https://metr.org/blog/2023-03-18-update-on-recent-evals/">2023</a> (the starting point for this dashboard’s analysis), there was just one formal organization, METR (then ARC), and some ad hoc individual red teamers that were involved in external evals for frontier AI models.</p>
        <p>Now, there are at least 13 private organizations, three government entities, and significant involvement by domain experts, individual red teamers, and uplift trial participants. These evaluators are filling gaps in key risk areas like CBRN risks, cybersecurity, scheming, and model welfare.</p>  
        ...
        <p>However, there are still relatively few organizations in the frontier model eval space. Further, many evaluators have fairly small staffs, and there are generally only 1–2 organizations that perform evals for a particular risk area. Given the high volume of models at or above the current frontier level <a href="https://epoch.ai/blog/model-counts-compute-thresholds">expected to be trained annually</a> over the next few years, rigorous testing across all risk areas will likely demand significantly more capacity. </p>
        <p>If the future of AI governance involves an “evaluations ecosystem” or a “regulatory market,” far more government support, public/private funding, and capable people will be needed to meet the growing need for third-party evals.</p>
      `,
    },
    {
      id: "analysis-2",
    ...
    },
    {
      ...
      content: `
        <p>In many cases, third-party evals are doing (at least part of) what they are supposed to do: finding issues the developer may not have been found themselves.</p>
        ...
        <p>This track record establishes that third-party evals can be effective in identifying and mitigating model risks in key areas, suggesting that external evaluators should likely be a key component of a frontier AI governance regime. Providing sufficient model access (to CoT, etc.) and information is also critical to allow the evaluators to do their job.</p>
        `,
    },
    {
      id: "analysis-4",
    ...
    },
    {
      ...
      content: `
        ...
        <p>So, while all US frontier labs have sought external evals voluntarily to some degree, there is significant variance as to how much and how seriously third parties are involved. Meta and xAI in particular show that we cannot rely on labs to voluntarily use third-party evals to a high standard; that there need to be formal, government-issued rules if we want all key models to be subject to robust and meaningful third-party testing.</p>
        `,
    },
    {
      id: "analysis-6",
      ...
      content: `
        <p>Labs often skip evals for incremental model updates. This is reasonable in principle, but the labs themselves determine what counts as “incremental,” and sometimes that determination is questionable.</p>
        ...
        <p>In other words, maybe benchmarks will not reliably tell us when a model has improved in real-world performance such that we need to re-evaluate it.</p>
    ...
    {
      id: "analysis-7",
      title: "Cherry-picking which models to evaluate creates gaps",
      content: `
        <p>Labs often evaluate some, but not all, models in a release. Usually this means evals are performed on just the largest/presumably most capable model and not the smaller/presumably less capable one(s). Once again, this seems reasonable in theory but poses two distinct problems.</p>
        <p>First, sometimes labs do not actually test the largest/most capable model. OpenAI’s GPT-5 <a href="https://cdn.openai.com/gpt-5-system-card.pdf#page=5">system card</a> “focuses primarily on gpt-5-thinking and gpt-5-main,” rather than the most powerful model, gpt-5-thinking-pro. OpenAI’s rationale was that, “[s]ince gpt-5-thinking-pro is gpt-5-thinking using a setting that makes use of parallel test time compute, we have determined that the results from our safety evaluations on gpt-5-thinking are strong proxies, and therefore we did not rerun these evaluations in the parallel test time compute setting.”</p>
        ...
        <p>However, this striking result suggests that we should consider requiring labs to test all models in a release in the future. Maybe, as in this instance, a smaller, distilled model happens to be very good at a particular task over larger models.</p>
      `,
    },
    {
      id: "analysis-8",
      ...
      content: `
        <p><a href="https://www.apolloresearch.ai/research/ai-behind-closed-doors-a-primer-on-the-governance-of-internal-deployment/">We know</a> that AI labs have more powerful models that they use internally before they are released publicly (some may never be released). We do not know if these models are evaluated, but if not, they should be.</p>
        <p>For instance, <a href="https://thezvi.substack.com/p/gpt-5s-are-alive-basic-facts-benchmarks?open=false#%C2%A7is-that-the-best-you-can-do">statements</a> from Sam Altman and from other AI policy experts support the idea that the GPT-5 models are not the most capable OpenAI has trained, just the most capable that it is publicly releasing at this time. Mowshowitz <a href="https://thezvi.substack.com/p/gpt-5s-are-alive-basic-facts-benchmarks?open=false#%C2%A7is-that-the-best-you-can-do">pointed out</a> that, “[i]f indeed there are superior internal models, those are the ones currently creating the most danger, and most in need of proper frontier testing.”</p>
